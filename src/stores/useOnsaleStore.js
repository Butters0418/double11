...
function generateTimeSlots() {
  ...
  if (currentTime < slot1000) {
    // 00:00~09:59 - 21:00 仍進行中
    slots = [
      { time: '21:00', status: 'now', isToday: false }, // 昨日 21:00
      { time: '10:00', status: 'ready', isToday: true },
...
function sortData(data) {
  const newData = data.map((period) => {
    ...
    const otherPds = [...period.products.slice(1)].sort(
      () => Math.random() - 0.5,
    );
...
const useOnsaleStore = create((set, get) => ({
  // 瘋搶資料
  onsaleData: [],
  isDataSorted: false, // 標記資料是否已排序
  // 取得瘋搶資料
  getData: async () => {
    // 如果資料已存在且已排序，不重新取得
    const { onsaleData, isDataSorted } = get();
    if (onsaleData.length > 0 && isDataSorted) {
      return;
    }
    ...
    set({ onsaleData: sortedData, isDataSorted: true });
  },
}));

...
export const SListContainer = styled.div`
  ${tw`w-full mt-2 text-sm text-white duration-200`}
  ...
  ul {
    ...
    ${(props) =>
      props.length > 0 ? tw`border border-solid border-white ` : tw``}
    li {
      ${tw`h-[32px] border-solid border-[#061539] border-t`}
      a {
        ${tw`flex items-center justify-center w-full h-full text-[#061539] font-[14px] font-medium duration-200`}
        ${tw`hover:(bg-white)`}
      }
    }
  }
...
export const SListWrap = styled.div`
  ${tw`relative -mt-[20px] space-y-[6px] overflow-hidden rounded-[8px] px-[6px] pb-[6px] pt-[32px] bg-cover`}
`;

// 左選上欄
export const STopList = styled.div`
  a {
    ${tw`relative flex flex-col h-[40px] items-center justify-center text-[18px] font-medium duration-200`}
    ${tw`hover:([background:radial-gradient(50%_50%_at_50%_50%,_#f5f2db,_#e7c77d_51.45%,_#66512e)])`}
  }
  a:hover {
  ...
  a::after {
    ${tw`content-[attr(after)] absolute bottom-0 left-1/2 -translate-x-1/2`}
    ${tw`mx-auto block h-[1px] w-[98px] bg-[#5868D1]`}
  }
